...
| `proxy.sessionDetail` | Adds a per-conversation breakdown (`sessions.items`) to `/teamclaude/status` and the dashboard: one row per conversation — the client session it belongs to and a digest naming the conversation within it, plus client, dimension values, pinned accounts, and the tokens it spent per weekly bucket. A session that fans out to subagents is one row per agent. A row's `id` is the composite `session/digest` key, with the halves repeated as the separate `session` and `conversation` fields; group by `session`, not `id`. **Off by default** — any holder of any proxy key can read status, so on a shared proxy this shows every consumer what every other consumer is working on. The aggregate `sessions` counts are unaffected and always present |
...
| `distributeSessions` | Spread concurrent conversations across equal-priority accounts, each pinned for cache reuse (`teamclaude distribute <on\|off\|adaptive>`) — the unit is a conversation and not the client session that holds it, so a session and its subagents spread rather than funnelling onto one account; one pin per weekly quota bucket, so a diversion in one model family leaves the others where they are (`false` = quota-driven rotation only, the default). Set to `"adaptive"` for a third mode that concentrates new conversations on the account with the least remaining weekly credit — tapering off as it nears its `switchThreshold` and backing off when it is congested. Subscription capacity comes from the OAuth profile tier; current utilization/reset comes from quota headers; burn rate and tolerated concurrency adapt to live traffic. See [Adaptive distribution](routing.md#adaptive-distribution). Applied live on config reload; turning it **off** drains — conversations already running keep their accounts and only new ones stop being distributed, so nobody loses a prompt cache mid-conversation. Session tracking and readout is always on regardless — see [Session-aware routing](routing.md#session-aware-routing) |
...
| `defaultClientMode` | What `teamclaude run` and `teamclaude env` do when neither `--mitm` nor `--no-mitm` is given: `mitm` (the default) routes every request of the launched client through the proxy, which is what covers the hard-coded `api.anthropic.com` endpoints and the Codex CLI; `base-url` sets only `ANTHROPIC_BASE_URL`, so a shell set up with `eval "$(teamclaude env)"` does not send its other tools (`gh`, `git`, package managers) through the proxy. A flag always wins over the setting. Toggled from the settings screen (**g** → Client mode); read from disk by each launch |
...
| `accounts[].switchThreshold` | Per-account override of the fleet-wide `switchThreshold`: a number, or a per-bucket table like `{ "unified7dFable": 0.8 }`. Resolution is per bucket — the account table's entry for that bucket, else the account's own `default` (or bare number), else the fleet's `switchThreshold` for that bucket — so a table without `default` only overrides the buckets it lists and inherits the rest from the fleet. Still a *preference*, exactly like the fleet setting: the all-exhausted revalidation probe can override it too. That is what keeps it a different setting from the hard `accounts[].maxUsage` cap below. No CLI editor — hand-edit and reload, same as `maxUsage`. Applied live on reload. See [Per-account thresholds](quota.md#per-account-thresholds) |
...
| `accounts[].bankedResets` | Claude banked resets you see in claude.ai Settings > Usage, which Anthropic does not list to the proxy: `[{ "expiresAt": "2026-10-22", "title": "..." }]`. A bare date is the start of that day, local time. Shown in the dashboard's Resets view with a Chat alert when added and one within 24 hours of expiry. No CLI editor — hand-edit and reload. See [Banked Claude resets](reset-tracking.md#banked-claude-resets) |
...
| --- | --- | --- |
| `TEAMCLAUDE_UPSTREAM_HEADERS_TIMEOUT_MS` | `120000`, Codex accounts `300000` | Max wait for upstream **response headers** (time-to-first-byte). Cleared the instant headers arrive, so a long streaming body is never cut. Streamed completions deliver first byte in seconds; a non-streaming (`stream:false`) request that legitimately generates for longer than this could trip it — raise it for such callers. Requests forwarded to a Codex account start from `300000` instead, because the ChatGPT backend holds the head open while the model reasons (see [Codex accounts](accounts.md#codex-accounts-experimental)). Setting this variable replaces both figures, for every account |
...
| `TEAMCLAUDE_HEADERLESS_429_RETRY_DELAY_MS` | `2000` | How long to wait before the single retry of a **headerless** (request-scoped) 429 — one carrying no `retry-after` and no `anthropic-ratelimit-*` headers. Retried once per request, for every provider — after a failover hop it stays on the account the hop landed on; if the retry is refused too the 429 goes to the client, just later. Lower it if you would rather have the 429 sooner than have the transient absorbed, or set it to `0` to disable the retry altogether, so a headerless 429 goes back to the client as soon as the failover hop (if any) has been refused too. Unset, empty or unparseable means the default — see [the two kinds of 429](routing.md#the-two-kinds-of-429) |
