...
The event stays recorded on the account. The first request that can be sent both there and to the cursor's account acts on it, ranked on the window governing that request. A request that also names an advisor model is admitted on its main model alone. It can spend the event onto an account that cannot serve its advisor, then be diverted for that request. The cursor is left where a request with no advisor would have put it. The flag is not in the quota state the proxy saves, and the poll that raised it already cleared the window, so a restart drops the event. Startup then places traffic by the ranking any start uses, owed nothing, the cursor included.
