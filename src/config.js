...
function lockIsStale(lockPath) {
  ...
  try {
    ...
    if (error.code === 'ENOENT') return false; // released under us; the retry takes it
  ...
  for (;;) {
    try {
      ...
      if (error.code !== 'EEXIST') {
        console.error(`[TeamClaude] Cannot create ${lockPath} (${error.code || error.message}); writing the config without it`);
        return false;
      }
    }
 ...
 * @param {(config: Record<string, any>) => any|Promise<any>} updater
 */
